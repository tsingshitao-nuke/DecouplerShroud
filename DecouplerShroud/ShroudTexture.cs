...
namespace DecouplerShroud {
	class ShroudTexture {
		...
		public string name = Localizer.Format("#LOC_DecouplerShroud_6");
        public string displayName = Localizer.Format("#LOC_DecouplerShroud_6");
		...
		public static void LoadTextures() {
			...
			if (shroudTextures == null) {
				...
				foreach(ConfigNode node in GameDatabase.Instance.GetConfigNodes("ShroudTexture"))  // NO_LOCALIZATION
				{
					if (node.HasValue("base")) {
						string texBaseName = node.GetValue("base").Trim(); // NO_LOCALIZATION
